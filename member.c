...
void memberLogo() {
	ENTER; ENTER; ENTER;
	makeMiddle("                         .^7??7^.                         ", PAGE_WIDTH); ENTER;
	makeMiddle("                        ?#&GPPG##Y.                       ", PAGE_WIDTH); ENTER;
	makeMiddle("         .:^:.         P@P:    .J@B:         :^:.         ", PAGE_WIDTH); ENTER;
	makeMiddle("       ~P##B##P~      ~@#.       5@Y      :5##B##G7       ", PAGE_WIDTH); ENTER;
	makeMiddle("      7@#!...!#@?     ?@G        ?@G     ~&@7:..~G@5      ", PAGE_WIDTH); ENTER;
	makeMiddle("     .#@~     ^@&:    ^@&:       P@J     P@J     .#@~     ", PAGE_WIDTH); ENTER;
	makeMiddle("     .&@^     :@@:     J@B~.   :5@G.     G@7      B@!     ", PAGE_WIDTH); ENTER;
	makeMiddle("      Y@G:   .5@5       ~#@#GGB@@J       7@#^    ?@B.     ", PAGE_WIDTH); ENTER;
	makeMiddle("       ?@&GPP&@Y.   .:~?P&B7~!~5&BY!^.    !&@BPP#@P.      ", PAGE_WIDTH); ENTER;
	makeMiddle("   :~7YB&G7?75&P..JGB#BPJ~      :75B##BP~ ?&B?77Y##P?~^.  ", PAGE_WIDTH); ENTER;
	makeMiddle(" ~B&BG57:     :^ G@5^:              .:!&@~.^     .~JPB##Y ", PAGE_WIDTH); ENTER;
	makeMiddle(".&@!            ~@#                    J@P            .B@7", PAGE_WIDTH); ENTER;
	makeMiddle("^@@J??????????! !@&???????????????????7P@G ~??????????7B@Y", PAGE_WIDTH); ENTER;
	makeMiddle(".JY55555555555J .JY5555555555555555555555! !555555555555Y~", PAGE_WIDTH); ENTER;
	ENTER; ENTER;
}
