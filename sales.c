...
//	tableHeaderForDisplayingSalesRecords();
//	for (int i = 0; i < loopCount; i++) {
//		printf("\n| %s | %-30s | %d | %-12s | %-20s | $%.2f | $%-6.2f | %02d/%02d/%04d |\n", line[i].salesID, line[i].itemName, line[i].itemQuantity, line[i].paymentMethod, line[i].creditCardDetails, line[i].salesTax, line[i].salesTotal, line[i].salesDate.day, line[i].salesDate.month, line[i].salesDate.year);
//		printf("%s", "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
//	}
//	printf("\n\n");
//	system("pause");
...
////void main() {
////	FILE* file = fopen("sales.bin", "wb");
////	SalesRecord row[] = { {"S001","Vitalite Vitamin A",3,"Credit Card","3289-3289-1020-2132",0.23,20.53,2,2018,21},{"S002","Baiyes Calcium",2,"Cash","-",0.43,10.32,5,2011,23},{"S003","Calcium Magnesium Tablet",6,"Cash","-",0.12,2.31,6,2011,1},{"S004","Vitalite Vitamin C",6,"Credit Card","6489-9302-1390-8929",1.93,30.87,1,2015,12},{"S005","Vitalite Vitamin C",5,"Credit Card", "1134-2189-8329-2891",4.59,56.78,3,2020,1}};
////	//SalesRecord row[5];
////	fwrite(row, sizeof(row), 1, file);
////	//for(int i = 0; i < 5; i++){
////		//printf("\n%s | %-10s | %d | %-12s | %-20s | %.2f | %-6.2f | %02d/%02d/%04d |", row[i].salesID, row[i].itemName, row[i].itemQuantity, row[i].paymentMethod, row[i].creditCardDetails, row[i].salesTax, row[i].salesTotal, row[i].salesDate.day, row[i].salesDate.month, row[i].salesDate.year);
